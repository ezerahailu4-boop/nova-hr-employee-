...
// Returns all admin Telegram IDs: env ADMIN_CHAT_ID + any in admins table
async function getAdminIds(): Promise<number[]> {
  ...
  if (supabaseAdmin) {
    try {
      ...
      data?.forEach((r: any) => { if (r.telegram_id) ids.add(Number(r.telegram_id)) })
    } catch { /* non-fatal */ }
  }
...
async function notifyAdmins(message: string, extra?: Record<string, any>) {
  const ids = await getAdminIds()
  await Promise.allSettled(ids.map(id => sendMessage(id, message, extra)))
}
...
async function handleWebAppData(message: any) {
  ...
  await notifyAdmins(adminText, { parse_mode: "Markdown", reply_markup: replyMarkup })
...
async function handleCvFile(message: any) {
  ...
  if (adminIds.length) {
    ...
    const replyMarkup = subId
      ...
      : undefined
    await Promise.allSettled(adminIds.map(id => forwardFileToAdmin(id, chatId, message.message_id, caption, { reply_markup: replyMarkup })))
  }
    ...
    if (cbData.startsWith("confirm_") || cbData.startsWith("decline_")) {
      ...
      if (supabaseAdmin) {
        ...
        if (sub) {
          ...
          await notifyAdmins(adminNote, { parse_mode: "Markdown" })
        }
      }
      return
    }
